...
function isValidInput(str) {
  ...
  const targetCount = passwordInput.password.split("").reduce((count, char) => {
  ...
  return (
    targetCount <= passwordInput.policy.maxCount &&
...
function isValidAlternateInput(str) {
  ...
  return (
    ((passwordInput.password[passwordInput.policy.firstPosition] == target) ^
      (passwordInput.password[passwordInput.policy.secondPosition] == target)) >
    0
  );
}
